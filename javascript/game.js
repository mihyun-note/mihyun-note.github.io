...
function chip_click() { //도움말 클릭 시
    
    var chip = document.getElementById('chip_text');
    console.log("칩 클릭");
...
function input(){ //도움말에 이름을 입력했을 시
...
function onoff() { //전원 온오프 버튼을 눌렀을 시

    var audio = document.getElementById('audio_button');
    var audio_backg = document.getElementById('audio_backg');
    audio_backg.volume = 0.4;
    
    var onoff_off = document.getElementById("onoff_off");
    var onoff_on = document.getElementById("onoff_on");
    ...
    console.log("전원버튼을 누르셨습니다.");
    console.log(num);
    
    if (num %2 == 1){ //켜졌을 때

        onoff_off.style.display = 'none';
        onoff_on.style.display = 'block';
        ...
        document.body.style.background = "url('./img/BackgroundNintendo_r.png') no-repeat 0 0";

        view.classList.remove('remove_play');
        view.classList.add('container');
        num++;
        
        console.log(temp);
        if (temp == undefined){
            document.getElementById("output").textContent = '이름이 뭐냐멍?'

            setTimeout(function(){
                document.getElementById("output").textContent = '도움말에서 이름을 알려주라멍!'
            },5000);

        }else{
            document.getElementById("output").textContent = temp + '님 안녕하멍!'
           
            
            setTimeout(function(){
                document.getElementById("output").textContent = temp + '님 지금 뭐하멍?'
            },8000);

            setTimeout(function(){
                document.getElementById("output").textContent = temp + '님 간식사달라멍'
            },8000);
        }
        audio_backg.play();
        audio_backg.loop = true;

        return;
    } else { //꺼졌을 때
        
        onoff_off.style.display = 'block';
        onoff_on.style.display = 'none';
        ...
        document.body.style.background = "url('./img/BackgroundNintendo_r.png') no-repeat 0 0";
        play1.classList.add('remove_play');
        play2.style.display = 'none';
        play3.classList.add('remove_play');

        audio_backg.pause();
        audio_backg.currentTime = 0;
        num++;
    }
}
...
function game_start(){ //첫번째 게임을 클릭했을 시
    var dog = document.getElementById('dog');
    dog.style.display = 'none';
    var view = document.getElementById('view');
    var play1 = document.getElementById('play1');
    var audio_game = document.getElementById('audio_game');
    audio_game.volume = 0.3;
    audio_game.play();
    console.log('게임클릭');

    view.classList.add('remove_play'); //선택화면 지우기
    play1.classList.remove('remove_play');

}
function game2_start(){ //두번째 게임을 클릭했을 시
    var dog = document.getElementById('dog');
    dog.style.display = 'none';
    var audio_game = document.getElementById('audio_game');
    ...
    console.log('게임2클릭');
    view.classList.add('remove_play'); //선택화면 지우기
    play2.classList.remove('remove_play');
                ...
                constructor(scene,x,y){
                    ...
                    display(){
                        if (num %2 == 1){
                            ...
                            canvas1.style.display = 'none'; //게임기가 종료되었을 시 게임 종료
                        }
                ...
                class Diorama {
                    ...
                    initCanvas( parameters ) {
                        ...
                        this.W = canvas1.width = parameters.width || 256;
                        this.H = canvas1.height = parameters.height || 256;
                        this.scale = parameters.scale || 1;
                        this.full = false;
                        this.ctx.imageSmoothingEnabled = false;
                        canvas1.classList.add("crisp");
                    ...
                    setScale() {
                        canvas1.style.maxWidth = this.W * this.scale + "px";
                        canvas1.style.maxHeight = this.H * this.scale + "px";
                    ...
                    fullScreen() {
                        ...
                        if (!this.full) {
                            ...
                            // reset
                            canvas1.style.maxWidth = "";
                            canvas1.style.maxHeight = "";
                            canvas1.style.width = "";
...
function game3_start(){ //세번째 게임을 클릭했을 때
    var dog = document.getElementById('dog');
    dog.style.display = 'none';
    var play3 = document.getElementById('play3');
    var audio_game = document.getElementById('audio_game');
    ...
    console.log('게임3클릭');
    view.classList.add('remove_play'); //선택화면 지우기
    play3.classList.remove('remove_play');
}

function audio_switch(){ //버튼을 클릭했을 때 나는 소리
    var audio = document.getElementById('audio_switch');

    audio.play();
}

function audio_cat(){ //고양이 위에 마우스를 올렸을 때 나는 소리
    var audio = document.getElementById('audio_cat');

    audio.play();
}
function audio_dog(){ //강아지 위에 마우스를 올렸을 때 나는 소리
    var audio = document.getElementById('audio_dog');
